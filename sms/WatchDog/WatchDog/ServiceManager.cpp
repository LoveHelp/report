...
{
	TCHAR scAppName[] = { _T("WATCHDOG") };
	TCHAR szFilePath[MAX_PATH] = { 0 };
	TCHAR szServWatch[32] = { 0 };
	//default
	_stprintf_s(m_servInfo.name, _T("SmServ"));
	_stprintf_s(m_servInfo.desc, _T("SMS Service"));
	_stprintf_s(szFilePath, _T("%S"), iniFile);

	GetPrivateProfileString(scAppName, _T("name"), _T("WatchDog"), m_servInfo.name, sizeof(m_servInfo.name), szFilePath);
	GetPrivateProfileString(scAppName, _T("desc"), _T("WatchDog For SMS Service"), m_servInfo.desc, sizeof(m_servInfo.desc), szFilePath);
	GetPrivateProfileString(_T("SERVICE"), _T("desc"), _T("SMS Service"), szServWatch, sizeof(szServWatch), szFilePath);

	_stprintf_s(m_servInfo.desc, _T("%s %s"), m_servInfo.desc, szServWatch);
	//lstrcat(m_servInfo.desc, szServWatch);
}
