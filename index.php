...
class wechatCallbackapiTest
{
    ...
    private function receiveText($object)
    {
        ...
        }else if(strstr($keyword, "报汛")){
			//include_once 'conn.php';
			$codeArr = array("B201601", "B201602", "B201603");
			$code = substr($keyword, 10, 7);
			$time = substr($keyword, 18, 6);
			//$current_time = date('y-m-d h:i:s');
			$current_time = strtotime(date('ymd'));
			$time = strtotime($time);
			if(!in_array($code, $codeArr, true)){
				$content = '组织机构代码错误，上报汛情失败！';
			}else if($time > $current_time){
				$content = 'time:' . $time . ' current_time:' . $current_time . '时间大于当前时间，上报汛情失败！';
			}else{
				$createtime = date('Y-m-d h:i:s', time());
				mysql_connect("122.114.144.156","enedu","enedu987");
				mysql_select_db("waterbureau");
				$sql = "insert into water (fromuser, content, createtime) values ('" . $fromuser . "', '" . $keyword . "', '" . $createtime . "')";
				$res = mysql_query($sql);
				if($res){
					$content = 'code:' . $code . ' time:' . $time . ' current_time:' . $current_time . '上报汛情成功！';
				}else{
					$content = $sql . $res . '添加汛情失败！';
				}
			}
			//$code = iconv('gbk', 'UTF-8', $str);
		}else{
